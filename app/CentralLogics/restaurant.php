...
class RestaurantLogic
{
    ...
    public static function get_latest_restaurants($limit = 10, $offset = 1, $zone_id)
    {
        $paginator = Restaurant::selectRaw('*, IF((opening_time < "'.now()->format('H:i').'" and closeing_time > "'.now()->format('H:i').'"), true, false) as open')
        ->with(['discount'=>function($q){
            return $q->validate();
    ...
    public static function get_popular_restaurants($limit = 10, $offset = 1, $zone_id)
    {
        $paginator = Restaurant::selectRaw('*, IF((opening_time < "'.now()->format('H:i').'" and closeing_time > "'.now()->format('H:i').'"), true, false) as open')
        ->with(['discount'=>function($q){
            return $q->validate();
        }])->where('zone_id', $zone_id)->where('store_type_id', 1)
        ->Active()
        ->withCount('orders')
    ...
    public static function search_restaurants($name, $zone_id, $limit = 10, $offset = 1)
    {
        ...
        })->active()->paginate($limit, ['*'], 'page', $offset);
